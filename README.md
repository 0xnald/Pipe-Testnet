...
This guide offers step-by-step instructions for setting up and configuring the POP Cache Node on Linux systems using the provided binaries.
* You must be whitelisted and receive an email with instructions to be eligible for node rewards.
* New users can signup [here](https://airtable.com/apph9N7T0WlrPqnyc/pagSLmmUFNFbnKVZh/form) and wait until you receive an email with invite code.
...
2. Run this command in your terminal to check your system architecture and determine the correct file to download.
```
uname -m
```
   If it returns something like x86_64, you're using a 64-bit architecture, so you should go with the Linux x64 version.

   If it returns aarch64, you're using an ARM64 architecture, and should choose the Linux ARM64 version.

3. Once Downloaded use termius SFTP feature to drag and drop the downloaded file in `/opt/popcache` you created in step 3
...
---
- Done !! Feel free to follow and ask question
- Twitter - https://x.com/0xnald
